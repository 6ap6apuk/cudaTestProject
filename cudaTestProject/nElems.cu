	...
	float* AX, float* AY, int nt, int N)
{
	...
	{
		if (j != id) // проверка самодействи€
		{
			xx = X[j + sh] - X[id + sh]; yy = Y[j + sh] - Y[id + sh];
			rr = sqrtf(xx * xx + yy * yy);
			if (rr > 0.01f) // минимальное рассто€ние 0.01 м
			{
	...
	int nt, int N, int N_block)
{
	...
	__shared__ float Xs[256]; __shared__ float Ys[256]; // выделение раздел€емой пам€ти
	for (int i = 0; i < N_block; i++) // основной цикл по блокам
	{
		Xs[threadIdx.x] = X[threadIdx.x + i * blockDim.x + sh]; // копирование из глобальной
		Ys[threadIdx.x] = Y[threadIdx.x + i * blockDim.x + sh]; // в раздел€емую пам€ть
		//__syncthreads(); // синхронизаци€
		for (int j = 0; j < blockDim.x; j++) // вычислительна€ часть
		{
...
// GPU-вариант. ѕересчет координат
...
// CPU Ц вариант. ¬ычисление ускорени€
...
// CPU-вариант. ѕересчет координат
...
int main(int argc, char* argv[]) {
	...
	cudaEvent_t start, stop;
		// »нициализаци€ событий CUDA
	cudaEventCreate(&start);
	cudaEventCreate(&stop);

	int N = 10240; // число частиц (2-й вариант 20480)
	int NT = 10; // число шагов по времени (дл€ анимации - 800)
	...
	for (int j = 1; j < NT; j++) {
		...
		// ѕроверка ошибок после каждого €дра
	...
	// —инхронизаци€ перед замером времени
	...
	auto cpu_start = std::chrono::high_resolution_clock::now(); // »справлено
	...
	timerValueCPU = std::chrono::duration<float, std::milli>(cpu_end - cpu_start).count(); // »справлено
